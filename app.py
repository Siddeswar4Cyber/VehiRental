...
def add_vehicle():
    try:
        if request.method == 'POST':
            ...
            try:
                print(reg_no, type_id, location, status)
    ...
    if request.method == 'POST':
        ...
        if request.form['action'] == 'Edit_Record':
            ...
            try:
                ...
                print("Insert Record of maintenance")
                cursor.execute("UPDATE vehicles SET next_maintenance_date = %s WHERE reg_no = %s", (next_date, vehicle_reg_no))
                print("Update next_maintenance_date of vehicle")
                db.commit()
                return redirect(url_for('maintenance'))
    ...
    if request.method == 'POST':   
        try: 
            ...
            try:
                ...
                db.rollback()
                return render_template('error.html',error_message=e)
            print(pickup_date,pickup_time,return_date,return_time,id,transaction_id)
